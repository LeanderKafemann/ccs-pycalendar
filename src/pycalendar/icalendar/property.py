...
class Property(PropertyBase):
    sDefaultValueTypeMap: Dict[str, int] = {
        ...
        definitions.cICalProperty_EXRULE: Value.VALUETYPE_RECUR,
    ...
    sVariant: str = "ical"
    sValue: str = definitions.cICalParameter_VALUE
    sText: str = definitions.cICalValue_TEXT
    ...
    def __init__(self, name: Optional[str] = None, value: Any = None, valuetype: Any = None) -> None:
        ...
        if value is None:
            ...
            self._init_attr_value_text(value.decode("utf-8"), valuetype if valuetype else self.sDefaultValueTypeMap.get(self.mName.upper(), Value.VALUETYPE_UNKNOWN))
            ...
            if name and name.upper() == definitions.cICalProperty_REQUEST_STATUS:
    ...
    def getRecurrenceValue(self) -> Optional[RecurrenceValue]:
    ...
    def _init_attr_value_requeststatus(self, reqstatus: Any) -> None:
        self.mValue = RequestStatusValue(reqstatus)
        self.setupValueParameter()

    def _init_attr_value_datetime(self, dt: DateTime) -> None:
    ...
    def _init_attr_value_datetimelist(self, dtl: List[DateTime]) -> None:
    ...
    def _init_attr_value_periodlist(self, periodlist: List[Period]) -> None:
        self.mValue = MultiValue(Value.VALUETYPE_PERIOD)
        for period in periodlist:
            self.mValue.addValue(PeriodValue(period))
        self.setupValueParameter()

    def _init_attr_value_duration(self, du: Duration) -> None:
        self.mValue = DurationValue(value=du)
        self.setupValueParameter()

    def _init_attr_value_period(self, pe: Period) -> None:
        self.mValue = PeriodValue(value=pe)
        self.setupValueParameter()

    def _init_attr_value_recur(self, recur: Recurrence) -> None:
